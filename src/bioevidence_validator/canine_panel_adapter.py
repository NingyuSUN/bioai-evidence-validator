...
from .engine import validate_record
...
def _truthy(value: Any) -> bool:
    return str(value).strip().casefold() in {"true", "1", "yes", "verified", "pass"}
...
def _requested_uses(manifest: dict[str, Any], source: str) -> list[str]:
    ...
    if not uses:
        raise ValueError(f"No requested uses configured for source {source!r}")
...
def _make_record(source: dict[str, Any], resolved: dict[str, Any], concept: dict[str, Any],
    ...
    evidence_items = [
        {
            ...
            "extracted_text": f"source={source['source']}; raw_label={source['source_raw_labels']}",
    ...
    evidence_ids = [source_item_id, concept_item_id]
    if override:
...
def export_canine_panel(database: Path, manifest_path: Path, output_dir: Path,
    ...
    for field in ("database_title", "database_version", "retrieved_at", "ontology_version"):
        if not manifest.get(field):
            raise ValueError(f"Adapter manifest is missing {field}")
    ...
    with closing(sqlite3.connect(database.as_uri() + "?mode=ro", uri=True)) as db:
        ...
        missing_source = REQUIRED_SOURCE_COLUMNS - _columns(db, "source_records")
        missing_concept = REQUIRED_CONCEPT_COLUMNS - _columns(db, "concepts")
        if missing_source or missing_concept:
            raise ValueError(
                "Database schema is missing required columns: "
                + ", ".join(sorted(missing_source | missing_concept))
            )
        sources = sorted(_rows(db, "source_records"), key=lambda row: row["record_key"])
        if limit is not None:
            sources = sources[:limit]
        resolved = {row["record_key"]: row for row in _rows(db, "resolved_source_names")}
        concepts = {row["source_concept_id"]: row for row in _rows(db, "concepts")}
        overrides = ({row["record_key"]: row for row in _rows(db, "name_link_overrides")}
    ...
    if after_hash != before_hash:
        raise RuntimeError("Input database changed during export")
